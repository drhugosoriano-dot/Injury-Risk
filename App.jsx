...
export default function App() {
  ...
  useEffect(() => {
    async function cargar() {
      ...
      let localJ = [], localE = {}, localD = {};
      if (local) {
        localJ = local.jugadores; localE = local.evaluaciones; localD = local.evalData;
        if (localJ.length > 0)              setJugadores(localJ);
        if (Object.keys(localE).length > 0) setEvaluaciones(localE);
        if (Object.keys(localD).length > 0) setEvalData(localD);
      }
      setDbReady(true);
      setSyncing(true);
      try {
        ...
        if (data) {
          ...
          const mergedJ = [...fbJ, ...localJ.filter(j => !fbIds.has(j.id))];
          const mergedE = deepMerge(fbE, localE);
          const mergedD = deepMerge(fbD, localD);
          setJugadores(mergedJ);
          setEvaluaciones(mergedE);
          setEvalData(mergedD);
          lsGuardar(mergedJ, mergedE, mergedD);
        }
        setSyncOk(true);
  ...
  async function guardarEnFirebase(j, e, d) {
    lsGuardar(j, e, d);
    try { await fbSet("cancunfc", "plantel", { jugadores: j||[], evaluaciones: e||{}, evalData: d||{} }); } catch(e2) {}
  }
  ...
  async function sincronizar() {
    ...
    try {
      ...
      if (data) {
        ...
        const fbD = data.evalData || {};
        const fbIds = new Set(fbJ.map(j => j.id));
        const mergedJ = [...fbJ, ...jugadores.filter(j => !fbIds.has(j.id))];
        const mergedE = deepMerge(fbE, evaluaciones);
        const mergedD = deepMerge(fbD, evalData);
        setJugadores(mergedJ);
        setEvaluaciones(mergedE);
        setEvalData(mergedD);
        lsGuardar(mergedJ, mergedE, mergedD);
        fbSet("cancunfc", "plantel", { jugadores: mergedJ, evaluaciones: mergedE, evalData: mergedD });
        setDebugMsg("OK: "+mergedJ.length+" jugs, "+Object.keys(mergedD).length+" evals");

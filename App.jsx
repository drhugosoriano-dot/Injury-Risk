...
// Guarda lista de jugadores + IDs borrados (escritura directa sin leer)
async function fbSetJugadores(jugadores, delIds=[]) {
  await setDoc(doc(db,"cancunfc","plantel"), {
    jugadores:  JSON.stringify(jugadores||[]),
    deletedIds: JSON.stringify(delIds||[]),
...
async function fbGetAll() {
  ...
  const deletedIds = parseField(snap.data().deletedIds, []);
  ...
  return { jugadores, evaluaciones, evalData, deletedIds };
}
...
// ─── SCREEN: PLAYERS ─────────────────────────────────────────────────────────
function ScreenPlayers({ area, evaluador, selfie, jugadores, evaluaciones, sesion, onSelect, onNuevo, onBack, onHome, onPredictor, onPrint, onBorrar }) {
            ...
            return (
              ...
              >
                ...
                <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:3 }}>
                  ...
                  <button onClick={e=>{e.stopPropagation();onBorrar(j);}} style={{ background:"#3a1a1a", border:"none", color:"#ef4444", borderRadius:5, padding:"3px 8px", cursor:"pointer", fontFamily:"monospace", fontSize:9, letterSpacing:1 }}>🗑 BORRAR</button>
      ...
      {(() => {
        ...
        return (
          <div style={{ marginTop:20 }}>
            ...
            <div style={{ display:"flex", justifyContent:"flex-end" }}>
              <button onClick={() => { if (!ok) return; setSaved(true); ctx.guardarPaso(ctx.jugador?.id, ctx.area?.paso, d); setTimeout(()=>ctx.onBack(),1200); }}
                style={{ background:ok?"#1a3a22":"#e0d9d0", border:"none", color:ok?"#c8f044":"#b0a898", borderRadius:10, padding:"12px 28px", cursor:ok?"pointer":"not-allowed", fontFamily:"'Barlow Condensed',sans-serif", fontWeight:700, fontSize:16, letterSpacing:2, boxShadow:ok?"3px 3px 0 #0d1f1440":"none" }}>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    fbGetAll().then(fb => {
      if (fb && fb.jugadores.length > 0) {
        // Merge Firebase con localStorage (no borrar datos locales)
        const lsNow = lsGet();
        const fbIds = new Set(fb.jugadores.map(j => j.id));
        const deletedSet = new Set(fb.deletedIds || []);
        const soloLocales = lsNow.j.filter(j => !fbIds.has(j.id) && !deletedSet.has(j.id));
        const mergedJ = [...fb.jugadores, ...soloLocales];
        const mergedE = { ...lsNow.e };
        for (const jId in fb.evaluaciones) {
          mergedE[jId] = { ...(lsNow.e[jId]||{}), ...fb.evaluaciones[jId] };
        }
        const mergedD = { ...lsNow.d };
        for (const jId in fb.evalData) {
          mergedD[jId] = { ...(lsNow.d[jId]||{}), ...fb.evalData[jId] };
        }
        setDeletedIds(fb.deletedIds || []);
        setJugadores(mergedJ);
        setEvaluaciones(mergedE);
        setEvalData(mergedD);
        lsSet(mergedJ, mergedE, mergedD);
      }
      setSyncOk(true);
    }).catch(() => setSyncOk(false))
      .finally(() => { setSyncing(false); setTimeout(()=>setSyncOk(null),3000); });

    // Listener en tiempo real — actualiza automaticamente sin tocar sync
    const unsubPlantel = onSnapshot(doc(db,"cancunfc","plantel"), async (snap) => {
      if (!snap.exists()) return;
      try {
        const raw = snap.data();
        const fbJ = parseField(raw.jugadores, []);
        const deletedIds = new Set(parseField(raw.deletedIds, []));
        const fbE = {}, fbD = {};
        for (const j of fbJ) {
          const ps = await getDoc(doc(db,"cancunfc","p_"+j.id));
          if (ps.exists()) {
            fbE[j.id] = parseField(ps.data().evaluaciones, {});
            fbD[j.id] = parseField(ps.data().evalData, {});
          }
        }
        setJugadores(prev => {
          const fbIds = new Set(fbJ.map(j => j.id));
          const soloLocales = prev.filter(j => !fbIds.has(j.id) && !deletedIds.has(j.id));
          return [...fbJ, ...soloLocales];
        });
        setEvaluaciones(prev => {
          const m = { ...prev };
          for (const jId in fbE) { m[jId] = { ...(prev[jId]||{}), ...fbE[jId] }; }
          for (const id of deletedIds) { delete m[id]; }
          return m;
        });
        setEvalData(prev => {
          const m = { ...prev };
          for (const jId in fbD) { m[jId] = { ...(prev[jId]||{}), ...fbD[jId] }; }
          for (const id of deletedIds) { delete m[id]; }
          return m;
        });
      } catch(e) {}
  ...
  async function guardarEnFirebase(j, e, d) {
    lsSet(j, e, d);
    try { await fbSetJugadores(j, deletedIds); } catch(x) { console.error("Error guardando jugadores:", x); }
  }
  ...
  async function sincronizar() {
    ...
    try {
      ...
      if (fb) {
        // NUNCA borrar jugadores locales — solo agregar los que no fueron borrados
        const fbIds = new Set(fb.jugadores.map(j => j.id));
        const deletedSet = new Set(fb.deletedIds || []);
        const soloLocales = jugadores.filter(j => !fbIds.has(j.id) && !deletedSet.has(j.id));
        const mergedJ = [...fb.jugadores, ...soloLocales];
        // Deep merge evaluaciones y evalData por jugador
        const mergedE = { ...evaluaciones };
        for (const jId in fb.evaluaciones) {
          mergedE[jId] = { ...(evaluaciones[jId]||{}), ...fb.evaluaciones[jId] };
        }
        const mergedD = { ...evalData };
        for (const jId in fb.evalData) {
          mergedD[jId] = { ...(evalData[jId]||{}), ...fb.evalData[jId] };
        }
        const mergedDelIds = [...new Set([...deletedIds, ...(fb.deletedIds||[])])];
        setDeletedIds(mergedDelIds);
        setJugadores(mergedJ);
        setEvaluaciones(mergedE);
        setEvalData(mergedD);
        lsSet(mergedJ, mergedE, mergedD);
        // Subir resultado fusionado a Firebase
        fbSetJugadores(mergedJ, [...new Set([...deletedIds, ...(fb.deletedIds||[])])]);
        for (const j of soloLocales) {
          fbGuardarPaso(j.id, mergedE[j.id]||{}, mergedD[j.id]||{});
        }
        setDebugMsg(mergedJ.length+" jugs · "+Object.keys(mergedD).length+" evals");
      } else { setDebugMsg("Sin datos en Firebase"); }
      setSyncOk(true);
    } catch(err) { setSyncOk(false); setDebugMsg("Error: "+err.message); }
    setSyncing(false);
  ...
  function handleBorrarJugador(j) {
    if (!window.confirm("¿Borrar a "+j.nombre+"? Esta acción no se puede deshacer.")) return;
    const nuevosJ = jugadores.filter(x => x.id !== j.id);
    const nuevasE = { ...evaluaciones }; delete nuevasE[j.id];
    const nuevosD = { ...evalData };    delete nuevosD[j.id];
    const newDelIds = [...new Set([...deletedIds, j.id])];
    setJugadores(nuevosJ);
    setEvaluaciones(nuevasE);
    setEvalData(nuevosD);
    setDeletedIds(newDelIds);
    lsSet(nuevosJ, nuevasE, nuevosD);
    fbSetJugadores(nuevosJ, newDelIds);
  }

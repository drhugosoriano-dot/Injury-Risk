...
// Guarda lista de jugadores + IDs borrados
async function fbSetJugadores(jugadores, addDeletedIds=[]) {
  try {
    const snap = await getDoc(doc(db,"cancunfc","plantel"));
    const prev = snap.exists() ? parseField(snap.data().deletedIds, []) : [];
    const allDeleted = [...new Set([...prev, ...addDeletedIds])];
    await setDoc(doc(db,"cancunfc","plantel"), {
      jugadores:  JSON.stringify(jugadores||[]),
      deletedIds: JSON.stringify(allDeleted),
...
export default function App() {
  ...
  function handleBorrarJugador(j) {
    ...
    fbSetJugadores(nuevosJ, [j.id]); // registra ID como borrado
  }

...
export default function App() {
  ...
  useEffect(() => {
    ...
    fbGetAll().then(fb => {
      if (fb && fb.jugadores.length > 0) {
        ...
        const soloLocales = lsNow.j.filter(j => !fbIds.has(j.id));
  ...
  async function sincronizar() {
    ...
    try {
      ...
      if (fb) {
        // NUNCA borrar jugadores locales — solo agregar los de Firebase
        const fbIds = new Set(fb.jugadores.map(j => j.id));
        const soloLocales = jugadores.filter(j => !fbIds.has(j.id));
        const mergedJ = [...fb.jugadores, ...soloLocales];

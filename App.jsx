...
async function fbGet(collection, docId) {
  try {
    ...
    const raw = snap.data();
    function parse(val, def) {
      if (!val) return def;
      if (typeof val === "string") { try { return JSON.parse(val); } catch(e) { return def; } }
      return val;
    }
    return {
      jugadores:    parse(raw.jugadores,    []),
      evaluaciones: parse(raw.evaluaciones, {}),
      evalData:     parse(raw.evalData,     {}),
    };
...
async function fbSet(collection, docId, data) {
  try {
    await setDoc(doc(db, collection, docId), {
      ...
      evaluaciones: JSON.stringify(data.evaluaciones || {}),
      evalData:     JSON.stringify(data.evalData     || {}),
...
export default function App() {
  ...
  function lsGuardar(j, e, d) {
    try {
      localStorage.setItem("cfc_jugadores",    JSON.stringify(j || []));
      localStorage.setItem("cfc_evaluaciones", JSON.stringify(e || {}));
      localStorage.setItem("cfc_evaldata",     JSON.stringify(d || {}));
    } catch(err) { console.error("localStorage error:", err); }
  }
  function lsCargar() {
    try {
      const j = localStorage.getItem("cfc_jugadores");
      const e = localStorage.getItem("cfc_evaluaciones");
      const d = localStorage.getItem("cfc_evaldata");
      return {
        jugadores:    j ? JSON.parse(j) : [],
        evaluaciones: e ? JSON.parse(e) : {},
        evalData:     d ? JSON.parse(d) : {},
      };
    } catch(err) { return null; }
  }

  // ── Cargar datos al iniciar — localStorage primero, luego Firebase ─────────
  useEffect(() => {
    async function cargar() {
      // 1. Cargar localStorage primero (instantáneo)
      const local = lsCargar();
      if (local) {
        if (local.jugadores.length > 0)                setJugadores(local.jugadores);
        if (Object.keys(local.evaluaciones).length > 0) setEvaluaciones(local.evaluaciones);
        if (Object.keys(local.evalData).length > 0)     setEvalData(local.evalData);
      }
      setDbReady(true); // Mostrar app inmediatamente con datos locales

      // 2. Intentar Firebase en background
      setSyncing(true);
      try {
        const data = await fbGet("cancunfc", "plantel");
        if (data) {
          const fbJ = data.jugadores || [];
          const fbE = data.evaluaciones || {};
          const fbD = data.evalData || {};
          // Fusionar: Firebase + local (local prevalece si más reciente)
          if (fbJ.length > 0) {
            setJugadores(prev => {
              const fbIds = new Set(fbJ.map(j => j.id));
              const localOnly = prev.filter(j => !fbIds.has(j.id));
              const merged = [...fbJ, ...localOnly];
              lsGuardar(merged, fbE, fbD);
              return merged;
            });
            // Deep merge por jugador — no perder pasos locales
            setEvaluaciones(prev => {
              const m = { ...fbE };
              for (const jId in prev) { m[jId] = { ...(fbE[jId]||{}), ...(prev[jId]||{}) }; }
              return m;
            });
            setEvalData(prev => {
              const m = { ...fbD };
              for (const jId in prev) { m[jId] = { ...(fbD[jId]||{}), ...(prev[jId]||{}) }; }
              return m;
            });
          }
        }
  ...
  async function guardarEnFirebase(j, e, d) {
    ...
    lsGuardar(j, e, d);
    // Luego intentar Firebase en background
    try {
      await fbSet("cancunfc", "plantel", { jugadores: j||[], evaluaciones: e||{}, evalData: d||{} });
    } catch(e2) { console.error("Error Firebase:", e2); }
  }

  // ── Sincronizar manualmente desde Firebase ────────────────────────────────
  ...
  async function sincronizar() {
    ...
    try {
      ...
      if (data) {
        ...
        const fbE = data.evaluaciones || {};
        const fbD = data.evalData || {};
        setDebugMsg("FB: "+fbJ.length+" jugs, evalKeys: "+Object.keys(fbD).length);
        setJugadores(prev => {
          const fbIds = new Set(fbJ.map(j => j.id));
          const localOnly = prev.filter(j => !fbIds.has(j.id));
          return [...fbJ, ...localOnly];
        });
        // Deep merge: combinar pasos de Firebase Y locales por jugador
        setEvaluaciones(prev => {
          const merged = { ...fbE };
          for (const jId in prev) {
            merged[jId] = { ...(fbE[jId]||{}), ...(prev[jId]||{}) };
          }
          return merged;
        });
        setEvalData(prev => {
          const merged = { ...fbD };
          for (const jId in prev) {
            merged[jId] = { ...(fbD[jId]||{}), ...(prev[jId]||{}) };
          }
          return merged;
        });
        lsGuardar(fbJ, {...fbE}, {...fbD});
      } else { setDebugMsg("Firebase sin datos"); }
      setSyncOk(true);
    } catch(e) { setSyncOk(false); setDebugMsg("Error: "+e.message); }
    setSyncing(false);
    setTimeout(() => setSyncOk(null), 3000);
  }
  ...
  // Guarda evalData Y evaluaciones en una sola operacion atomica
  function guardarPaso(jId, paso, data) {
    ...
    guardarEnFirebase(jugadores, nuevasEval, nuevosData);
  }

...
export default function App() {
  ...
  useEffect(() => {
    ...
    setDbReady(true);
    // 2. Firebase en background
    setSyncing(true);
    fbGet("cancunfc","plantel").then(fb => {
      if (fb && fb.jugadores && fb.jugadores.length > 0) {
  ...
  async function guardarEnFirebase(j, e, d) {
    lsSet(j, e, d);
    try { await fbSet("cancunfc","plantel",{jugadores:j||[],evaluaciones:e||{},evalData:d||{}}); } catch(x) {}
  }
  ...
  async function sincronizar() {
    ...
    try {
      const fb = await fbGet("cancunfc","plantel");
      if (fb) {
        const j = fb.jugadores    || [];
        const e = fb.evaluaciones || {};
        const d = fb.evalData     || {};
        setJugadores(j);
        setEvaluaciones(e);
        setEvalData(d);
        lsSet(j, e, d);
        setDebugMsg(j.length+" jugs · "+Object.keys(d).length+" evals");

...
// ─── SCREEN: HOME ─────────────────────────────────────────────────────────────
function ScreenHome({ onArea, sesion, onCerrarSesion, jugadores, msgAcceso, sincronizar, syncing, syncOk }) {
  return (
    <>
      <link href={FONT_LINK} rel="stylesheet" />
      ...
      {(() => {
        ...
        return (
          <div style={{ marginTop:20 }}>
            ...
            <div style={{ display:"flex", justifyContent:"flex-end" }}>
              <button onClick={() => { if (!ok) return; setSaved(true); ctx.saveEvalData(ctx.jugador?.id, 2, his); ctx.markPasoCompleto(ctx.jugador?.id, 2); setTimeout(()=>ctx.onBack(),1200); }}
                style={{ background:ok?"#1a3a22":"#e0d9d0", border:"none", color:ok?"#c8f044":"#b0a898", borderRadius:10, padding:"12px 28px", cursor:ok?"pointer":"not-allowed", fontFamily:"'Barlow Condensed',sans-serif", fontWeight:700, fontSize:16, letterSpacing:2, boxShadow:ok?"3px 3px 0 #0d1f1440":"none" }}>
      ...
      {(() => {
        ...
        return (
          <div style={{ marginTop:20 }}>
            ...
            <div style={{ display:"flex", justifyContent:"flex-end" }}>
              <button onClick={() => { if (!ok) return; setSaved(true); ctx.saveEvalData(ctx.jugador?.id, ctx.area?.paso, dat); ctx.markPasoCompleto(ctx.jugador?.id, ctx.area?.paso); setTimeout(()=>ctx.onBack(),1200); }}
                style={{ background:ok?"#1a3a22":"#e0d9d0", border:"none", color:ok?"#c8f044":"#b0a898", borderRadius:10, padding:"12px 28px", cursor:ok?"pointer":"not-allowed", fontFamily:"'Barlow Condensed',sans-serif", fontWeight:700, fontSize:16, letterSpacing:2, boxShadow:ok?"3px 3px 0 #0d1f1440":"none" }}>
...
export default function App() {
  ...
  useEffect(() => {
    async function cargar() {
      ...
      try {
        ...
        if (data) {
          if (data.jugadores && data.jugadores.length > 0) setJugadores(data.jugadores);
          if (data.evaluaciones && Object.keys(data.evaluaciones).length > 0) setEvaluaciones(data.evaluaciones);
          if (data.evalData && Object.keys(data.evalData).length > 0) setEvalData(data.evalData);
        }
        setSyncOk(true);
      } catch(e) {
  ...
  async function sincronizar() {
    ...
    try {
      const data = await fbGet("cancunfc", "plantel");
      if (data) {
        // Merge: keep local players not in Firebase, add Firebase players not local
        const fbJugadores = data.jugadores || [];
        const fbEval = data.evaluaciones || {};
        const fbData = data.evalData || {};
        
        setJugadores(prev => {
          const localIds = new Set(prev.map(j => j.id));
          const fbIds = new Set(fbJugadores.map(j => j.id));
          // Combine: all FB jugadores + any local-only jugadores
          const localOnly = prev.filter(j => !fbIds.has(j.id));
          return [...fbJugadores, ...localOnly];
        });
        setEvaluaciones(prev => ({ ...prev, ...fbEval }));
        setEvalData(prev => ({ ...prev, ...fbData }));
      }
      setSyncOk(true);
  ...
  return (
    <>
      ...
      {screen==="home"&&<ScreenHome onArea={id=>{setMsgAcceso(""); handleArea(id);}} sesion={sesion} onCerrarSesion={cerrarSesion} jugadores={jugadores} msgAcceso={msgAcceso} sincronizar={sincronizar} syncing={syncing} syncOk={syncOk} />}
